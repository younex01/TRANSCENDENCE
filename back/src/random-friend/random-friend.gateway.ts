...
export class RandomFriendGateway implements OnGatewayDisconnect {
  ...
  async handleConnection(socket: Socket): Promise<void> {
    ...
    if (availibleRoomId)
    {
      ...
      this.players[this.id].push({id: socket.id, playerNb: 2, x: 880, y: 175 ,score:0, width: 20, height: 100,name:"player2",giveUp: false,db_id: "",pic: ""});
    ...
    else
    {
      ...
      this.players[this.id].push({id: newRoomId, playerNb: 1, x:0, y: 175,score: 0,width: 20, height: 100,name: "player1",giveUp:false,db_id: "",pic: ""});
    }
  }
  ...
  {
    this.gameService.handle_id(id, client.id,this.game);
  }
