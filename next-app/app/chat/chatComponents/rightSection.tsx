...
export const RightSection = (props:any) => {
    ...
    const fetchChatGroups = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          console.log("data.data", (await axios.get(`http://localhost:3000/chat/getIsMuted?userId=${props.userData.id}&groupId=${conversationId}`)).data)
          console.log(conversationId)
          // console.log(data.data.mutedUsers)
  ...
  console.log("---------------. isMutedisMutedisMutedisMuteddata", isMuted)
  ...
  function handleChannelCommands(arg:string, data:any, index:any) {
    props.socket.emit(arg, {userId: data.id, groupId: groupData.id})
    props.socket.on("refresh", () =>{
      setRefresh(!refresh);
      console.log("ldakhl dyal socket.on")
  ...
  // if (groupData.status === "Punlic")
  // return (
  //   <div>dd</div>
  // )
  // if (groupData.status === "Protected")
  // return (
  //   <div>dd</div>
  // )
  // if (groupData.status === "Private")
  // return (
  //   <div>dd</div>
  // )
  function checkIfUserMuted(userId:string){
    console.log("--------------------" , userId)
    console.log("groupData.mutedUsers.includes(userId); --------------------" , groupData.mutedUsers.includes(userId))
      ...
      <div className='h-full w-full flex flex-1 relative z-10'>
        <div className='w-full h-full relative'>
          ...
          <div className='chatHeight overflow-hidden overflow-y-visible overflow-x-hidden '>
                  ...
                  {(missage.userId === props.userData.id) && 
                    <div className='flex pr-[16px] self-end lg:max-w-[50%] max-w-[100%] '>
                      <div className='flex flex-col items-end mb-1'>
                        <div key={groupData.id} className='rounded-b-[24px] rounded-tl-[24px] w-fit font-normal text-[18px] font-sans-only break-all bg-gray-100 p-[16px] lg:min-w-[100%] max-w-[70%]'>{missage.content}  </div>
                      </div>
                    </div>
                  }
                  {(missage.userId !== props.userData.id) && 
                    <div className='flex pl-[16px] pt-[4px] gap-[16px] lg:max-w-[50%] max-w-[70%]'>
                      {index > 0 && (missage.userId === messages[index - 1].userId) && 
                        <div key={groupData.id} className='ml-[62px] self-start rounded-b-[24px] rounded-tr-[24px] w-fit font-normal text-[18px] font-sans-only break-all bg-gray-100 p-[16px] lg:w-fit max-w-[100%]'> {missage.content} </div>
                      }
                          ...
                          <div className='mt-7 border-black flex'>
                            ...
                            <div className='rounded-tr-[24px] rounded-b-[24px]  '>
                              <div className='flex h-[45px] items-start '>
                                ...
                                {(groupData.owner == missage.userId) && <div className='text-[14px] mt-[2px] rounded-[9px] font-light text-white py-[6px] px-[10px] bg-[#7239D3]' >owner</div> }
                              </div>
                              <div key={groupData.id} className='rounded-b-[24px] rounded-tr-[24px] w-fit font-normal text-[18px] font-sans-only break-all bg-gray-100 p-[16px] lg:w-fit max-w-[100%]'>
                                {missage.content}
                              </div>
                            </div>
                          </div>
            ...
            {(isMuted > 0) ? 
            (
              <div className='sendBox flex justify-between items-center w-full h-[100px] absolute bottom-0 border-t-[2px] bg-opacity-[52%] overflow-hidden bg-red-700'> 
            ...
            :
            (
              <div className='sendBox flex justify-between items-center w-full h-[100px] absolute bottom-0 border-t-[2px] bg-opacity-[52%] overflow-hidden bg-green-800'> 
        ...
        <div className="fixed top-0 right-0 z-20 w-full backdrop-blur-[8px] h-full transition-all duration-500 transform translate-x-full shadow-lg peer-checked:translate-x-0">
                        ...
                        <div className='flex overflow-visible'>
                          ...
                          <div className='z-20'>
                            ...
                            {(groupData.owner == members.id) ? <div className='text-gray-400 text-[14px] self-start'>Owner</div>
                             : 
                             <div className='text-gray-400 text-[14px] self-start'>status </div>}
                          </div>
                          { memberSettings[index] && (
                            ...
                            <div className='font-semibold absolute right-12 top-4 z-50 bg-[#e6ebfe] flex flex-col border-[1px] rounded-[10px] p-1 '>
                              ...
                              <div className='hover:text-[#8d94af] cursor-pointer py-2 px-2 rounded-[8px]' onClick={() => {handleChannelCommands("block",groupData.members[index], index)}}>block</div>
                          ...
                          {groupData.status === "Protected" ? (
                            <div className='flex flex-col items-start gap-2'>
                              ...
                              {changePassword && (
                                  ...
                                  <input className='searchBar w-[100%] h-[45px] pl-5 rounded-xl bg-gray-200 ' type="text" placeholder="Change Password" />
                                    <div className='self-end'>
                                      <button className='bg-[#495791]  text-white border-[1px] p-2 text-[16px] w-[80px] mt-2 rounded-[13px] font-medium hover:bg-[#313d6c] transition-all duration-500'>save</button>
                              ...
                              <button className='w-full p-2 bg-gray-200 text-[#495791] rounded-[10px] font-semibold hover:bg-gray-300 transition-all duration-500'>Set To Public</button>
                            </div>
                          ) : (
                            <div className='flex flex-col items-start gap-2'>
                            ...
                            {changePassword && (
                                <div className='w-full flex flex-col'>
                                  <input className='searchBar w-[100%] h-[45px] pl-5 rounded-xl bg-gray-200 ' type="text" placeholder="Set Password" />
          ...
          <div className='absolute bottom-10 right-4 bg-[#D1483E] hover:bg-red-700 rounded-[10px] transition-all duration-500'>
              <div className='flex px-4'>
                ...
                <button className='p-3 font-bold text-white'> Leave Group Chat</button>
              </div>
          </div>
        </div>
      </div>
