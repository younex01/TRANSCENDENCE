...
export default function createGroupChat(props:any) {
  ...
  const addGroupChat = async () => {
    if(channelName ) {
      ...
      try {
        if (avatar) {
          console.log("tzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz");
          const formData = new FormData();
          formData.append('file', avatar);
          console.log(avatar);
          const response2 = await axios.post('http://localhost:3000/chat/uploads', formData);
          console.log(photoPath);
          console.log(response2);
        }
        // setPhotoPath(response2);
      ...
      props.setChatConvos((prevchatConvos:any) => {
        const updatedConvos = [...prevchatConvos, groupChatInfo];
        setChannelPassWord("");
        setChannelName("");
        setPassWord("hidden");
          ...
          <div className='absolute mt-[10px] top-[91px] w-[100%] border-[1px] border-color1'></div>
            <div className='h-[70%] w-[80%] flex justify-around flex-col z-40'>
              ...
              <div className='flex justify-between w-[100%]'>
                ...
                <div>
                  <button className='text-[#D7D7D7] text-[20px] font-serif bg-opacity-[50%] h-[57px] w-[200px] border-[1px] rounded-[10px] border-color1 bg-[#6e7aad] relative' onClick={()=> setStatus(!status)}>{channelOption ?  channelOption : "Select Option"}
                  ...
                  {status && (
                    ...
                    <button className='hover:border-l hover:border-r border-white pl-2 pb-2 w-full flex' onClick={() => {setChannelOption("Protected"); setStatus(false), setPassWord("")}}>Protected</button>
                    <button className='hover:border-l hover:border-r border-white pl-2 pb-2 w-full flex' onClick={() => {setChannelOption("Private"); setStatus(false), setPassWord("hidden")}}>Private</button>
                    <button className='hover:border-l hover:border-r border-white pl-2 w-full flex' onClick={() => {setChannelOption("Public"); setStatus(false), setPassWord("hidden")}}>Public</button>
                    </div>
                  )}
                  </button> 
                </div>
            ...
            <div className='absolute bottom-5 right-9 font-[IBM-Plex-Serif] font-thin flex items-center justify-center'>
              ...
              <button className='w-[139px] h-[40px] bg-[#FFFFFF] bg-opacity-[41%] text-[30px] flex items-center justify-center rounded-[19px] opacity-[69%]' onClick={() => props.setNewGroup(false)}>Cancel</button>
            </div>
        </div>
      </div>
    </div>
